...
png(file = "plot2.png") ## Open PNG device; create 'plot2.png' in your working directory

#convert the Date and Time variables to Time class in R
idata.time <- strptime(paste(idata$Date,idata$Time),format = "%d/%m/%Y %H:%M:%S")

plot(idata.time,idata$Global_active_power,ylab = "Global Active Power (kilowatts)",xlab = "",type = "l")
